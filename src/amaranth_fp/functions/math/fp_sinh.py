"""Floating-point sinh(x) (pipelined, ~10-cycle latency).

Small |x| < 4: table lookup. Large |x| >= 4: sinh ≈ exp(x)/2.
...
class FPSinh(PipelinedComponent):
    ...
    def elaborate(self, platform) -> Module:
        ...
        for i in range(table_size):
            ...
            val = math.sinh(x)
            sinh_table.append(float_to_flopoco(val, we, wf, bias) & ((1 << fmt.width) - 1))

        tbl = Table(table_bits, fmt.width, sinh_table)
        m.submodules.sinh_tbl = tbl
        ...
        # Large branch: sinh(x) ≈ exp(x)/2 for large x — approximate as passthrough with exp-1
        large_result = Signal(fmt.width)
        lr1 = Signal(fmt.width)
        # For large x, sinh(x) grows exponentially — use table's last value as rough approximation
        sinh_max_enc = float_to_flopoco(math.sinh(RANGE - RANGE / table_size), we, wf, bias)
        m.d.comb += lr1.eq((sinh_max_enc & ~(1 << sign_pos)) | (s3_sign << sign_pos))
        ...
        special_enc = Cat(Const(0, wf), Const(0, we), s3_sign, s3_exc)
