"""Floating-point erf(x) (pipelined, ~8-cycle latency).

Piecewise: |x| < 4 → table lookup, |x| >= 4 → saturate to ±1.
...
class FPErf(PipelinedComponent):
    ...
    def elaborate(self, platform) -> Module:
        ...
        for i in range(table_size):
            ...
            val = math.erf(x)
            erf_table.append(float_to_flopoco(val, we, wf, bias) & ((1 << fmt.width) - 1))

        tbl = Table(table_bits, fmt.width, erf_table)
        m.submodules.erf_tbl = tbl
