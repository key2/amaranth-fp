"""Floating-point sinc(x) = sin(x)/x, sinc(0) = 1 (pipelined, ~10-cycle latency).

Table lookup on [0, 4), large |x| → 0.
BranchMux with zero-detect.
"""
...
class FPSinc(PipelinedComponent):
    ...
    def elaborate(self, platform) -> Module:
        ...
        for i in range(table_size):
            ...
            val = 1.0 if x == 0 else math.sin(x) / x
            sinc_table.append(float_to_flopoco(val, we, wf, bias) & ((1 << fmt.width) - 1))

        tbl = Table(table_bits, fmt.width, sinc_table)
        m.submodules.sinc_tbl = tbl
        ...
        with m.Elif(s1_exc == 0b00):  # sinc(0) = 1
            m.d.comb += [s1_exc_out.eq(0b01), s1_special.eq(1)]
        with m.Elif(s1_exc == 0b10):  # sinc(±inf) = 0
        ...
        # sinc(x) = sinc(-x), always positive for table values (sinc can be negative but near 0)
        small_result = Signal(fmt.width)
        m.d.comb += small_result.eq(tbl_val)

        # Large: approximate sinc(x) → 0 for large x
        large_result = Signal(fmt.width)
        lr1 = Signal(fmt.width)
        m.d.comb += lr1.eq(0)  # zero encoding
